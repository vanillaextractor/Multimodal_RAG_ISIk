...
app = FastAPI(
    title="Multimodal RAG Pipeline (FastAPI + Groq)",
    description="A prototype for ingesting, chunking, and vectorizing PDF documents with Groq API for chat.",
)
...
class CustomPGRetriever(BaseRetriever):
    ...
    top_k: int = 15                      # expanded retrieval pool
    top_n_rerank: int = 3                # lowered to save Groq API tokens (max 8k TPM)
...
async def chat_with_docs(request: ChatRequest):
    ...
    try:
        ...
        session_data.append({
            ...
            "top_k": 3, # top_n_rerank value in main_groq.py
